...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    if ($_SESSION['role'] !== 'admin') {
        ...
        if ($name && $category && $quantity >= 0 && $location && $code) {
            ...
            $stmt->bind_param("ssiss", $name, $category, $quantity, $location, $code);
            if ($stmt->execute()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_product'])) {
    if ($_SESSION['role'] !== 'admin') {
        ...
        $delete_code = trim($_POST['delete_id']);
        $delete_quantity = intval($_POST['delete_quantity']);
        ...
        if ($delete_code && $delete_quantity > 0) {
            ...
            if ($current_quantity >= $delete_quantity) {
                ...
                // Обновление количества товара или удаление
                if ($new_quantity > 0) {
...
$sql = "SELECT id, name, category, quantity, location, код FROM products";
if ($searchQuery) {
